---
type: summary
title: "Vandereyken 2023 — Methods and applications for single-cell and spatial multi-omics"
source: "[[00-Sources/papers/Methods and applications for single-cell and spatial multi-omics]]"
aliases: ["Vandereyken 2023 review", "Voet-lab multi-omics review"]
tags: [review, single-cell-multiomics, spatial-multiomics, G&T-seq, scNMT-seq, sci-CAR, Voet-lab, KU-Leuven]
created: 2026-05-13
updated: 2026-05-13
---

**Citation:** Vandereyken et al. (2023) — *Methods and applications for single-cell and spatial multi-omics* — *Nature Reviews Genetics*. [DOI](https://doi.org/10.1038/s41576-023-00580-2)

Vandereyken, Sifrim, Thienpont and Voet (KU Leuven) reviewed single-cell and spatial multi-omics methods. The single-cell methods section is organized by four design principles: (i) **physical separation before sequencing** (G&T-seq, SIDR-seq, DNTR-seq); (ii) preamplification-and-split (DR-seq); (iii) shared barcoding before separation; (iv) joint biochemistry. Covers genomics+transcriptomics, methylome+transcriptome, chromatin+transcriptome (sci-CAR), triple-omics (scNMT-seq), and emerging combinations including proteomics (CITE-seq/REAP-seq). Spatial section covers DBiT-seq, MERFISH, Stereo-seq, Slide-seq, Visium, and integrated transcriptome+protein assays. Discusses computational integration strategies (anchor-based, factor-analysis, deep learning).

## Why this matters

The most scDNA-aware multi-omics review of 2023 — Voet co-authored G&T-seq (Macaulay 2015) and is a primary architect of the scDNA+scRNA multimodal landscape. Strong §3.1 reference complementing Baysoy 2023 (less DNA-focused) and Heumos 2023 (analysis-focused). Provides the **design-principle taxonomy** (separation vs preamp-split vs joint biochemistry) that is useful for organizing the user's §3 structure. Existing bibkey `vandereyken2023` already present.

---
**Source:** [DOI](https://doi.org/10.1038/s41576-023-00580-2) · [PubMed](https://pubmed.ncbi.nlm.nih.gov/36864178/)

---
**Source:** [DOI](https://doi.org/10.1038/s41576-023-00580-2) · [PubMed](https://pubmed.ncbi.nlm.nih.gov/36864178/)

## Related

- [[10-Summaries/macaulay-2015-gt-seq]]
- [[10-Summaries/clark-2018-scnmt]]
- [[10-Summaries/baysoy-2023-multiomics-landscape]]
- [[10-Summaries/heumos-2023-best-practices]]
- [[20-Entities/thierry-voet]]
